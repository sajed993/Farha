  ...
  <div class="editor">
   ...
   <div class="controls">
    ...
    <div class="ctl-sec"><label>🎬 ${t().lAnim}</label><div class="opt-row">${anims}</div>
     ...
     <div class="opt-row">${Object.keys(LUXE_THEMES).map(k=>`<button class="opt prem-opt ${(S.c.anim==='luxe'&&(S.c.luxeTheme||'bab')===k)?'on':''}" style="border-color:var(--gold3);background:${(S.c.anim==='luxe'&&(S.c.luxeTheme||'bab')===k)?'linear-gradient(120deg,#C69A45,#9A7327)':'linear-gradient(120deg,#F5EEE0,#EFE6D3)'};color:${(S.c.anim==='luxe'&&(S.c.luxeTheme||'bab')===k)?'#fff':'#8A6420'};font-weight:800" onclick="setC('luxeTheme','${k}');setC('anim','luxe')">🚪 ${LUXE_THEMES[k].name[S.lang]||k}</button>`).join('')}</div>
     <div style="font-size:.72rem;color:#8A7A63;margin-top:6px">${S.lang==='ar'?'أبواب قصر تُفتح ثلاثيّة الأبعاد، ثم تظهر الأسماء حرفًا حرفًا':S.lang==='fr'?"Portes de palais en 3D, puis les noms lettre par lettre":'3D palace doors, then names letter by letter'}</div>
